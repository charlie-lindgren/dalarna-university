---
kurskod: GMT3K6
kursnamn: "Forskningsmetodik för ingenjörer"
course_name: "Research Methods for Engineers"
hp: 7,5 högskolepoäng
niva: "Grundnivå"
huvudomrade: "Ej huvudområde 1"
amne: "Maskinteknik"
amne_kod: "MTA"
institution: "IIT"
tags: [kursplan, MTA, IIT, stray, vilande]
cssclasses: [vilande]
scrape_hash: 2a2e8fe9886aacb8
url: https://www.du.se/sv/utbildning/kurser/kursplan/?code=GMT3K6
up: "[[Maskinteknik MOC]]"
---

# GMT3K6

[Kursplan på du.se →](https://www.du.se/sv/utbildning/kurser/kursplan/?code=GMT3K6)

**Kursnamn:** Forskningsmetodik för ingenjörer
**Course Name:** Research Methods for Engineers

- **Poäng:** 7,5 högskolepoäng
- **Nivå:** Grundnivå
- **Kursen kan ingå i följande huvudområde(n):** Ej huvudområde 1
- **Ämnestillhörighet:** Maskinteknik (MTA)
- **Fastställd:** Fastställd 2026-05-25 . Kursplanen gäller fr.o.m. 2026-05-25.

## Lärandemål

Efter godkänd kurs ska studenten kunna:

  - beskriva grunderna i forskningsprocessen och dess steg

  - förklara grundläggande vetenskapliga kvalitetskriterier

  - förklara etiska aspekter av forskning

  - planera ett tekniskt forskningsprojekt

  - föreslå och motivera val av metoder för insamling och analys av data

  - strukturera och skriva en vetenskaplig text.

## Innehåll

Kursen behandlar grunderna i forskningsmetodik för blivande ingenjörer. Processer och metoder för forskning studeras, liksom metoder för datainsamling och dataanalys. Vidare belyses etiska överväganden och riktlinjer samt grunderna i vetenskaplig kommunikation.

## Examinationsformer

- Inlämningsuppgift

  - Muntlig examination med individuell bedömning

## Betyg

Som betygsskala på hel kurs används U–G.

Betyg rapporteras enligt följande:

  - Inlämningsuppgift - 4,5 hp | U–G
  - Muntlig examination - 3 hp | U–G

## Förkunskapskrav

- Grundläggande behörighet

## Övrigt

Kursen kan inte ingå i en examen tillsammans med andra kurser med motsvarande innehåll.

Om studenten har ett beslut/rekommendation om riktat pedagogiskt stöd från Högskolan Dalarna på grund av funktionsnedsättning, har examinator rätt att anpassa examinationen. Examinator avgör utifrån kursplanens mål om examinationen kan anpassas i enlighet med beslutet/rekommendationen.

## English Version

### Learning Outcomes

On completion of the course, students will be able to:

  - describe the fundamentals of the research process and its stages

  - explain basic scientific quality criteria

  - explain ethical aspects of research

  - plan a technical research project

  - propose and justify the choice of methods for data collection and analysis

  - structure and write a scientific text.

### Course Content

The course covers the fundamentals of research methodology for engineering students. Research processes and methods are examined, together with methods for data collection and data analysis. Ethical considerations and guidelines are also addressed, as well as the basics of scientific communication.

### Assessment

- Assignment

  - Oral exam with individual assessment

### Grades

The grading scale used for the final course grade is U–G.

Grades are reported as follows:

  - Assignment - 4.5 Credits | U–G
  - Oral exam - 3 Credits | U–G

### Prerequisites

- General entry requirements
